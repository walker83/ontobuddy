...
| 构造 | 典型推导 | 不支持原因 / 状态 |
|------|---------|-------------------|
| `rdfs:domain` | x P y, P domain C ⟹ x a C | 未实现。对拍 case 04 唯一 FN 即此。**候选补强项**。 |
| `rdfs:range` | x P y, P range C ⟹ y a C | 未实现。同上。 |
...
| `owl:disjointWith` | 矛盾检测 | **未实现，且无一致性检查**（重要缺口）。 |
...
> **重要缺口**：`owl:disjointWith` 缺失意味着 myonto **不做一致性检测**。
> 即本体里写 `Cat disjointWith Dog` + `felix a Cat, a Dog` 时，推理机不会报错。
> 这是相比"多推几条三元组"对用户更有价值的特性，列为后续优先项。
...
```
汇总: Precision=1.0000  Recall=0.9412  F1=0.9697 (TP=16 FP=0 FN=1)
  FP vs owlrl = 0     ← soundness 保证（myonto 从不比标准多推）
  FN vs owlrl = 1     ← 唯一缺口：case 04 的 rdfs:range（known-unsupported）
```

**判据解读**：
- **FP == 0**（硬门禁）：4 个用例、16 条推导，**无一比 OWL 2 RL 多推**。
  这是 soundness 的客观证据。CI 可用 `make crosscheck --strict` 把它设为硬门禁。
- **Recall = 0.941**：唯一 FN 是 case 04 的 `principia a Work`（rdfs:range 推导），
  属 §3 记录的 known-unsupported 缺口。
- **在声明支持的 7 条规则范围内，recall = 100%**——完备性在承诺子集内成立。
...
| 该推的都推了吗（承诺范围内）？ | `make eval` 14 个 golden case | 每 case P=R=F1=1.0 |
